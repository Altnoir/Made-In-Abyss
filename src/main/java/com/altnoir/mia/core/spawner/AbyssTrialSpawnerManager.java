...
public class AbyssTrialSpawnerManager extends SimpleJsonResourceReloadListener {
            ...
            try {
                ...
                if (json.has("loot_tables")) {
                    ...
                    for (var lootElement : lootTablesArray) {
                        ...
                        var lootTableId =
                                ResourceLocation.parse(lootJson.get("loot_table").getAsString());
                ...
                if (json.has("entity_tables")) {
                    ...
                    for (var entityElement : entityTablesArray) {
                        ...
                        var entityId =
                                ResourceLocation.parse(entityJson.get("entity").getAsString());
                        ...
                        Map<Holder<Attribute>, AttributeModifier> attributeModifiers =
                                parseAttributeModifiers(entityJson);

                        entityTablesList.add(
                                new EntityTableInstance(
                                        entityType,
                                        weight,
                                        equipment,
                                        effects,
                ...
                var mobsPerPlayer =
                        json.has("mobs_per_player") ? json.get("mobs_per_player").getAsInt() : 1;
                var spawnPerTick =
                        json.has("spawn_per_tick") ? json.get("spawn_per_tick").getAsInt() : 1;
    ...
    private List<MobEffectInstance> parseEffects(JsonObject entityJson) {
        ...
        if (entityJson.has("effects")) {
            ...
            for (var effectElement : effectsArray) {
                ...
                var duration =
                        effectJson.has("duration") ? effectJson.get("duration").getAsInt() : -1;
                var amplifier =
                        effectJson.has("amplifier") ? effectJson.get("amplifier").getAsInt() : 0;
                var ambient = effectJson.has("ambient") && effectJson.get("ambient").getAsBoolean();
                var visible =
                        !effectJson.has("visible") || effectJson.get("visible").getAsBoolean();
                var showIcon =
                        !effectJson.has("show_icon") || effectJson.get("show_icon").getAsBoolean();
    ...
    private Map<Holder<Attribute>, AttributeModifier> parseAttributeModifiers(
        ...
        if (entityJson.has("attribute_modifiers")) {
            ...
            for (var modifierElement : modifiersArray) {
                ...
                var attributeId =
                        ResourceLocation.parse(modifierJson.get("attribute").getAsString());
                var modifierId = ResourceLocation.parse(modifierJson.get("id").getAsString());
                var amount = modifierJson.get("amount").getAsDouble();
                var operationStr =
                        modifierJson.has("operation")
                                ? modifierJson.get("operation").getAsString()
                                : "add_value";
