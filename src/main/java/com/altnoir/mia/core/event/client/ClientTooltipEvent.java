...
public class ClientTooltipEvent {
    ...
    public static final String TOOLTIP_MODIFIERS_ARTIFACT_MATERIAL =
            "tooltip.mia.modifiers.artifact_material";
    ...
    public static void onTooltip(ItemStack stack, List<Component> tooltip) {
        ...
        if (stack.is(MiaTags.Items.ARTIFACT_MODIFIERS_MATERIAL)) {
            ...
            if (recipeManager != null) {
                tooltip.add(
                        1,
                        Component.translatable(TOOLTIP_MODIFIERS_ARTIFACT_MATERIAL)
    ...
    private static List<Component> materialModifiers(
            ItemStack material, List<RecipeHolder<ArtifactSmithingRecipe>> recipes) {
        return recipes.stream()
                ...
                .map(
                        recipe -> {
                            ...
                            return formatAttributeModifier(
                                    attribute, minValue, maxValue, operation);
                        })
                .toList();
    }

    public static Component formatAttributeModifier(
        ...
        String valueText;
        if (isPercent
                || attribute.is(MiaAttributes.CRITICAL_HIT)
                || attribute.is(MiaAttributes.CRITICAL_HIT_DAMAGE)) {
