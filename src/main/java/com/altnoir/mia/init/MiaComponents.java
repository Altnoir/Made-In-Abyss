...
public class MiaComponents {
    public static final DeferredRegister<DataComponentType<?>> DATA_COMPONENT_TYPES =
            DeferredRegister.createDataComponents(Registries.DATA_COMPONENT_TYPE, MIA.MOD_ID);

    public static final DeferredHolder<
                    DataComponentType<?>, DataComponentType<ArtifactBundleInventoryComponent>>
            ARTIFACT_BUNDLE_INVENTORY =
                    register(
                            "artifact_bundle_inventory",
                            builder ->
                                    builder.persistent(ArtifactBundleInventoryComponent.CODEC)
                                            .networkSynchronized(
                                                    ArtifactBundleInventoryComponent.STREAM_CODEC));

    public static final DeferredHolder<
                    DataComponentType<?>, DataComponentType<ArtifactEnhancementComponent>>
            ARTIFACT_ENHANCEMENT =
                    register(
                            "artifact_enhancement",
                            builder ->
                                    builder.persistent(ArtifactEnhancementComponent.CODEC)
                                            .networkSynchronized(
                                                    ArtifactEnhancementComponent.STREAM_CODEC));
