...
public class MiaClientConfig {
    ...
    public static final ModConfigSpec.BooleanValue AUTO_HOOK =
            BUILDER.comment("Enable auto hook | 启用自动回收").define("hook.auto", false);
    private static final ModConfigSpec.EnumValue<CurseIconPosition> CURSE_ICON_POSITION =
            BUILDER.comment(
                            "Curse icon position (Default: MIDDLE) | 诅咒图标位置 (默认值: MIDDLE)",
    ...
    private static final ModConfigSpec.EnumValue<DisconnectButtonState> DISCONNECT_BUTTON_STATE =
            BUILDER.comment(
                            "Controls the disconnect button behavior (Default: DEFAULT) | 控制退出按钮行为"
                                    + " (默认值: DEFAULT)",
