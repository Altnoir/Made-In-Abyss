...
public record CaveFloorSearch(VerticalAnchor minY, int clearance, int horizontalSamples) {
                    ...
                    instance ->
                            instance.group(
                                            VerticalAnchor.CODEC
                                                    .fieldOf("min_y")
                                                    .forGetter(CaveFloorSearch::minY),
                                            Codec.intRange(1, 128)
                                                    .fieldOf("clearance")
                                                    .forGetter(CaveFloorSearch::clearance),
                                            Codec.intRange(1, 256)
                                                    .fieldOf("horizontal_samples")
                                                    .forGetter(CaveFloorSearch::horizontalSamples))
    ...
    public Optional<BlockPos> findStart(Structure.GenerationContext context, int maxStartY) {
        ...
        int minFloorY =
                Math.max(
                        this.minY.resolveY(heightContext),
                        context.heightAccessor().getMinBuildHeight());
        int boundedMaxStartY =
                Math.min(maxStartY, context.heightAccessor().getMaxBuildHeight() - this.clearance);
    ...
    public List<Integer> findStartYs(BlockColumn column, int minFloorY, int maxStartY) {
        ...
        for (int y = scanTopY; y >= minFloorY; y--) {
            ...
            if (startY <= maxStartY
                    ...
                    && state.isFaceSturdy(
                            EmptyBlockGetter.INSTANCE, floorPos.set(0, y, 0), Direction.UP)) {
                starts.add(startY);
            }
            dryAirRun = 0;
        }
