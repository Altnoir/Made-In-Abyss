...
public class MiaConfig {
    ...
    private static final ModConfigSpec.IntValue ABYSS_RADIUS =
            COMMON_BUILDER
     ...
     * 无尽锤的配置
     */
    private static final ModConfigSpec.IntValue BLAZE_REAP_EXPLOSION_COUNT =
            COMMON_BUILDER
                    .comment(
                            "The Boom counts of the Blaze REAP (Default: 4)| Blaze REAP武器攻击时的爆炸次数"
                                    + " (默认值: 4)")
                    .defineInRange("blaze_reap.explosion_count", 3, 1, 64);
    private static final ModConfigSpec.DoubleValue BLAZE_REAP_EXPLOSION_RADIUS =
            COMMON_BUILDER
                    .comment(
                            "The Boom radius of the Blaze REAP (Default: 2) | Blaze REAP武器爆炸范围"
                                    + " (默认值: 2)")
                    .defineInRange("blaze_reap.explosion_radius", 2.0, 0.1, 20.0);
    /**
     * 钩爪相关的配置
     */
    private static final ModConfigSpec.DoubleValue HOOK_MAX_DISTANCE =
            COMMON_BUILDER
                    .comment("Maximum hook distance in blocks (Default: 32.0) | 最大钩爪距离 (默认值: 32.0)")
                    .defineInRange("hook.max_distance", 32.0, 1.0, 256.0);
    private static final ModConfigSpec.DoubleValue HOOK_SHOOT_VELOCITY =
            COMMON_BUILDER
                    .comment("Hook shoot velocity (Default: 4.0) | 钩爪射击速度 (默认值: 4.0)")
                    .defineInRange("hook.shoot_velocity", 4.0, 0.5, 4.0);
    private static final ModConfigSpec.DoubleValue HOOK_PULL_VELOCITY =
            COMMON_BUILDER
                    .comment("Base pull velocity (Default: 0.3) | 基础拉取速度 (默认值: 0.2)")
                    .defineInRange("hook.pull_velocity", 0.2, 0.01, 2.0);
    private static final ModConfigSpec.DoubleValue HOOK_STOP_DISTANCE =
            COMMON_BUILDER
                    .comment(
                            "Distance at which the hook stops pulling the player (Default: 1.0) |"
                                    + " 钩爪停止拉取距离 (默认值: 1.0)")
                    .defineInRange("hook.stop_distance", 1.41421, 1.0, 10.0);
    private static final ModConfigSpec.DoubleValue HOOK_RETRACT_VELOCITY =
            COMMON_BUILDER
                    .comment("Hook retract velocity (Default: 0.75) | 钩爪收回速度 (默认值: 0.75)")
                    .defineInRange("hook.retract_velocity", 0.75, 0.1, 5.0);
    private static final ModConfigSpec.DoubleValue HOOK_RETRACT_DISTANCE =
            COMMON_BUILDER
                    .comment("Retract distance in blocks (Default: 4.0) | 回收距离 (默认值: 4.0)")
                    .defineInRange("hook.retract_distance", 4.0, 1.0, 16.0);
    private static final ModConfigSpec.DoubleValue HOOK_JUMP_BOOST =
            COMMON_BUILDER
     ...
     * 诅咒相关的配置
     */
    private static final ModConfigSpec.BooleanValue CURSE =
            SERVER_BUILDER
                    .comment("Whether to enable the Curse (Default: true) | 是否启用上升诅咒 (默认值: true)")
                    .define("curse", true);
    private static final ModConfigSpec.BooleanValue CURSE_GOD =
            SERVER_BUILDER
                    .comment(
                            "Whether to Curse the Creative and Spectator(Default: false) |"
                                    + " 是否诅咒创造模式和观察者模式 (默认值: false, 需要启用诅咒)")
                    .define("curse_god", false);
    /**
     * 探窟者信标相关的配置
     */
    private static final ModConfigSpec.IntValue CAVE_EXPLORER_BEACON_HORIZONTAL =
            SERVER_BUILDER
                    .comment(
                            "The horizontal range of the Cave Explorer Beacon, Beacon Level x"
                                    + " (Default: 10) + 10 | 探窟者信标水平半径,信标等级 x (默认值: 10) + 10")
                    .defineInRange("cave_explorer_beacon.horizontal", 10, 1, 1000);
    private static final ModConfigSpec.IntValue CAVE_EXPLORER_BEACON_VERTICAL =
            SERVER_BUILDER
                    .comment(
                            "The vertical range of the Cave Explorer Beacon, Beacon Level x"
                                    + " (Default: 5) + 5 | 探窟者信标垂直半径,信标等级 x (默认值: 5) + 5")
                    .defineInRange("cave_explorer_beacon.vertical", 5, 1, 1000);
    private static final ModConfigSpec.BooleanValue CAVE_EXPLORER_BEACON_MAX_VERTICAL =
            SERVER_BUILDER
                    .comment(
                            "Whether to set the vertical range of the Cave Explorer Beacon to the"
                                    + " world height (Default: false) | 是否把探窟者信标垂直半径设置为世界高度 (默认值:"
                                    + " false)")
                    .define("cave_explorer_beacon.max_vertical", false);
    /**
     * 深渊怪物等级相关的配置
     */
    private static final ModConfigSpec.BooleanValue ABYSS_MOB_LEVEL_SWITCH =
            SERVER_BUILDER
                    .comment(
                            "Switch to enable the Abyss Mob Level (Default: true) | 开启深渊怪物等级 (默认值:"
                                    + " true)")
                    .define("abyss_mob_level.switch", true);
    private static final ModConfigSpec.EnumValue<AbyssMobLevelIncreasingCurve>
            ABYSS_MOB_LEVEL_INCREASING_CURVE =
                    SERVER_BUILDER
                            .comment(
                                    "The increasing curve of the Abyss Mob Level (Default:"
                                            + " FromSlowToFast) | 深渊怪物等级增长曲线 (默认值: FromSlowToFast)")
                            .defineEnum(
                                    "abyss_mob_level.increasing_curve",
                                    AbyssMobLevelIncreasingCurve.FROM_FAST_TO_SLOW);

    public enum AbyssMobLevelIncreasingCurve {
        FROM_FAST_TO_SLOW,
        FROM_SLOW_TO_FAST
    }
