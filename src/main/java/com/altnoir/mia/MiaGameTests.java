...
public class MiaGameTests {
    ...
    public static void abyssWallPlanIsDeterministicSeparatedAndInRange(GameTestHelper helper) {
        ...
        List<AbyssWallCandidate> repeated =
                AbyssWallPlanner.createPlan(123456789L, config, cylinder);
        List<AbyssWallCandidate> otherSeed =
                AbyssWallPlanner.createPlan(-987654321L, config, cylinder);
        ...
        helper.assertTrue(
                first.size() >= 16 && first.size() <= 20,
                "default plan must retain approximately 16-20 candidates across its twenty height"
                        + " bands");
        long bufferedMinimumDistance = AbyssWallPlanConfig.DEFAULT.planningAnchorDistance();
        helper.assertValueEqual(
                AbyssWallPlanConfig.DEFAULT.maxWallAnchorOffset(),
                3,
                "the configured embedding value is the maximum radial offset");
        helper.assertValueEqual(
                bufferedMinimumDistance,
                74L,
                "planning distance reserves correction, embedding, and block rounding on both"
                        + " anchors");

        for (int i = 0; i < first.size(); i++) {
            AbyssWallCandidate candidate = first.get(i);
            helper.assertTrue(
                    candidate.y() >= -96 && candidate.y() <= 125,
                    "candidate Y must stay in the allowed windmill range");
            helper.assertTrue(
                    candidate.y() < -3 || candidate.y() > 25,
                    "candidate Y must avoid the horizontal transition shelf from -3 through 25");
            for (int j = 0; j < i; j++) {
                helper.assertTrue(
                        candidate.anchor().distSqr(first.get(j).anchor())
                                >= bufferedMinimumDistance * bufferedMinimumDistance,
                        "predicted anchors must reserve correction space around the final minimum"
                                + " separation");
                helper.assertFalse(
                        candidate.startChunk().equals(first.get(j).startChunk()),
                        "one structure cannot own two candidates in the same start chunk");
            }
        }
        Set<Integer> embedOffsets =
                first.stream()
                        .map(
                                candidate ->
                                        AbyssWallPlanner.wallAnchorOffset(
                                                123456789L, candidate, config))
                        .collect(Collectors.toSet());
        helper.assertTrue(
                embedOffsets.size() > 1, "deterministic embedding must vary between candidates");
        for (int offset : embedOffsets) {
            helper.assertTrue(
                    offset >= 0 && offset <= 3, "embedding must stay in the inclusive 0-3 range");
        }
    ...
    public static void abyssWallPlanCodecSerializesTuningKnobs(GameTestHelper helper) {
        JsonObject encoded =
                AbyssWallPlanConfig.CODEC
                        .encodeStart(JsonOps.INSTANCE, AbyssWallPlanConfig.DEFAULT)
                        .getOrThrow()
                        .getAsJsonObject();

        helper.assertValueEqual(
                encoded.get("max_y").getAsInt(),
                125,
                "the default data contract must temporarily disable windmills above Y=125");
        helper.assertValueEqual(
                encoded.get("forbidden_min_y").getAsInt(),
                -3,
                "the transition shelf lower bound must be serialized");
        helper.assertValueEqual(
                encoded.get("forbidden_max_y").getAsInt(),
                25,
                "the transition shelf upper bound must be serialized");
        helper.assertValueEqual(
                encoded.get("final_density_inward_search").getAsInt(),
                32,
                "the inward final-density search distance must be serialized");
        helper.assertValueEqual(
                encoded.get("final_density_tolerance").getAsInt(),
                3,
    ...
    public static void abyssWallPlanConfigControlsForbiddenYRange(GameTestHelper helper) {
        ...
        helper.assertValueEqual(
                ...
                List.of(),
                "a data-configured forbidden range covering the full height range must produce no"
                        + " candidates");
        helper.succeed();
    }
    ...
    public static void abyssWallPlanConfigControlsDensitySearch(GameTestHelper helper) {
        ...
        helper.assertValueEqual(
                configuredPrediction,
                92.0,
                "a wall within the configured inward search must be selected");
        helper.assertValueEqual(
                minimumSampledRadius[0], 88, "an inward search of 8 must begin at macroRadius - 8");
        helper.succeed();
    }
    ...
    public static void abyssWallDirectionsSelectStraightAndTiltRotations(GameTestHelper helper) {
        helper.assertValueEqual(
                AbyssWallPlanner.orientationFor(Math.toRadians(90)),
                new AbyssWallCandidate.Orientation(
                        AbyssWallCandidate.TemplateKind.STRAIGHT, Rotation.NONE),
                "south wall uses the authored straight orientation");
        helper.assertValueEqual(
                AbyssWallPlanner.orientationFor(Math.toRadians(45)),
                new AbyssWallCandidate.Orientation(
                        AbyssWallCandidate.TemplateKind.TILT, Rotation.NONE),
                "south-east wall uses the authored tilt orientation");
        helper.assertValueEqual(
                AbyssWallPlanner.orientationFor(Math.toRadians(180)),
                new AbyssWallCandidate.Orientation(
                        AbyssWallCandidate.TemplateKind.STRAIGHT, Rotation.CLOCKWISE_90),
                "west wall rotates the straight template clockwise");
        helper.assertValueEqual(
                AbyssWallPlanner.orientationFor(Math.toRadians(22.49)),
                new AbyssWallCandidate.Orientation(
                        AbyssWallCandidate.TemplateKind.STRAIGHT, Rotation.COUNTERCLOCKWISE_90),
                "angle below the sector boundary remains straight");
        helper.assertValueEqual(
                AbyssWallPlanner.orientationFor(Math.toRadians(22.51)),
                new AbyssWallCandidate.Orientation(
                        AbyssWallCandidate.TemplateKind.TILT, Rotation.NONE),
    ...
    public static void abyssWallRefinementRequiresStableAirToSolidBoundary(GameTestHelper helper) {
        OptionalInt boundary =
                AbyssWallPlanner.findFirstStableBoundary(180, 230, radius -> radius >= 205);
        OptionalInt missing = AbyssWallPlanner.findFirstStableBoundary(180, 230, radius -> false);
        OptionalInt lowerWall =
                AbyssWallPlanner.findRefinedBoundary(96.0, 16, radius -> radius >= 94);
        OptionalInt unrelatedOuterWall =
                AbyssWallPlanner.findRefinedBoundary(96.0, 16, radius -> radius >= 125);
        double inwardPrediction = AbyssWallPlanner.preferredRadius(96.0, 3, radius -> radius >= 90);
        double toleratedPrediction =
                AbyssWallPlanner.preferredRadius(96.0, 3, radius -> radius >= 99);
        double justOutsideTolerance =
                AbyssWallPlanner.preferredRadius(96.0, 3, radius -> radius >= 100);
        double distantOuterPrediction =
                AbyssWallPlanner.preferredRadius(96.0, 3, radius -> radius >= 125);
        double solidAtSearchStart =
                AbyssWallPlanner.preferredRadius(96.0, 3, radius -> radius >= 64);
        int[] minimumSampledRadius = {Integer.MAX_VALUE};
        double boundedPrediction =
                AbyssWallPlanner.preferredRadius(
                        96.0,
                        3,
                        radius -> {
                            minimumSampledRadius[0] = Math.min(minimumSampledRadius[0], radius);
        ...
        helper.assertValueEqual(
                missing, OptionalInt.empty(), "missing wall must reject the candidate");
        helper.assertValueEqual(
                lowerWall,
                OptionalInt.of(94),
                "nearby lower-layer wall must refine the macro radius");
        helper.assertValueEqual(
                unrelatedOuterWall,
                OptionalInt.empty(),
        ...
        helper.assertValueEqual(
                inwardPrediction,
                90.0,
                "an inward final-density wall must override the macro radius");
        helper.assertValueEqual(
                toleratedPrediction, 99.0, "the tolerance boundary must remain eligible");
        helper.assertValueEqual(
                justOutsideTolerance,
                96.0,
                "a final-density wall beyond tolerance must fall back to the macro radius");
        helper.assertValueEqual(
                distantOuterPrediction,
                96.0,
                "a distant outer wall must fall back to the macro radius");
        helper.assertTrue(
                Double.isNaN(solidAtSearchStart),
        ...
        helper.assertValueEqual(
                minimumSampledRadius[0],
                64,
                "final density must not be sampled more than 32 blocks inward from the macro"
                        + " radius");
        helper.succeed();
    }
    ...
    public static void abyssWallPlanFindsTheRealAbyssDensityWall(GameTestHelper helper) {
        ...
        var settings =
                helper.getLevel()
                        ...
                        .getOrThrow(MiaNoiseGeneratorSettings.THE_ABYSS)
                        .value();
        ...
        List<AbyssWallCandidate> candidates =
                AbyssWallPlanner.planFor(seed, randomState, AbyssWallPlanConfig.DEFAULT);

        helper.assertTrue(
                candidates.size() >= 16 && candidates.size() <= 20,
        ...
        for (AbyssWallCandidate candidate : candidates) {
            ...
            if (candidate.y() < -3) {
                helper.assertTrue(
                        candidate.predictedRadius() <= 99.0,
                        "lower-layer prediction must not select a wall beyond macro radius plus"
                                + " tolerance");
            }
        }
        helper.assertValueEqual(
        ...
        List<List<AbyssWallCandidate>> concurrentPlans =
                IntStream.range(0, 16)
                        .parallel()
                        .mapToObj(
                                ignored ->
                                        AbyssWallPlanner.planFor(
                                                concurrentSeed,
                                                concurrentRandomState,
                                                AbyssWallPlanConfig.DEFAULT))
                        .toList();
        for (List<AbyssWallCandidate> concurrentPlan : concurrentPlans) {
            helper.assertValueEqual(
                    concurrentPlan,
                    concurrentPlans.getFirst(),
    ...
    @PrefixGameTestTemplate(false)
    @GameTest(template = "gametest/amethyst_lamptube")
    public static void compassRuinStructuresUseTheirTemplateNamesAsRegistryIds(
    ...
    public static void compassRuinWorldgenKeepsBiomeAndPlacementContract(GameTestHelper helper) {
        ...
        assertBiomeTagEquals(
                helper,
                MiaTags.Biomes.HAS_ANCIENT_TRIAL_COMPASS_RUINS,
        ...
        assertRandomSpreadSet(
                helper,
                MiaStructureSets.ANCIENT_ROMAN_COMPASS_RUINS,
                32,
                8,
                70387318,
                Map.of("mia:ancient_roman_compass_ruins", 1));
        assertRandomSpreadSet(
                helper,
                MiaStructureSets.ANCIENT_TRIAL_COMPASS_RUINS,
                34,
                12,
                94251328,
                Map.of("mia:ancient_trial_compass_ruins", 1));
        assertRandomSpreadSet(
                helper,
                MiaStructureSets.ANCIENT_ANGKOR_COMPASS_RUINS,
                32,
                8,
                70387319,
                Map.of("mia:ancient_angkor_compass_ruins", 1));

        JsonObject trial = encodeStructure(helper, MiaStructures.ANCIENT_TRIAL_COMPASS_RUINS);
        helper.assertValueEqual(
                trial.get("step").getAsString(),
                GenerationStep.Decoration.UNDERGROUND_STRUCTURES.getName(),
                "trial generation step");
        JsonObject trialHeight = trial.getAsJsonObject("start_height");
        helper.assertValueEqual(
                trialHeight.getAsJsonObject("min_inclusive").get("absolute").getAsInt(),
                -40,
                "trial minimum Y");
        helper.assertValueEqual(
                trialHeight.getAsJsonObject("max_inclusive").get("absolute").getAsInt(),
                -20,
                "trial maximum Y");
        helper.succeed();
    }
    ...
    @PrefixGameTestTemplate(false)
    @GameTest(template = "gametest/amethyst_lamptube")
    public static void petrifiedShipTemplatesKeepTheirDirectionalJigsawContract(
            GameTestHelper helper) {
        assertJigsawTemplate(
                ...
                Set.of(
                        ...
                        new JigsawContract(
                                ...
                                "mia:petrified_ship/connect_right",
                                "mia:petrified_ship/connect_left",
                                "mia:petrified_ship/connect_left"),
                        new JigsawContract(
                                new BlockPos(3, 1, 16),
        ...
        assertJigsawTemplate(
                helper,
                "petrified_ship_part_1",
                new Vec3i(4, 3, 5),
                Set.of(branchEnd(new BlockPos(3, 0, 2))));
        assertJigsawTemplate(
                helper,
                "petrified_ship_part_2",
                new Vec3i(5, 4, 5),
                Set.of(branchEnd(new BlockPos(4, 1, 2))));
        assertJigsawTemplate(
                helper,
                "petrified_ship_part_3",
                new Vec3i(5, 4, 5),
                Set.of(branchEnd(new BlockPos(4, 1, 2))));
        assertJigsawTemplate(
                helper,
                "petrified_ship_part_4",
                new Vec3i(5, 2, 3),
                Set.of(branchEnd(new BlockPos(4, 1, 1))));
        assertJigsawTemplate(
                helper,
                "petrified_ship_part_5",
                new Vec3i(6, 4, 4),
    ...
    public static void petrifiedShipPoolsKeepDirectionalTerminationWeights(GameTestHelper helper) {
        ...
        assertTemplatePool(
                helper,
                "petrified_ship/start",
                "mia:petrified_ship/main",
                Map.of(
                        "mia:petrified_ship_body_small", 1,
                        "mia:petrified_ship_body_big", 1));
        assertTemplatePool(
                helper,
                "petrified_ship/connect_left",
                "mia:petrified_ship/left_or_right",
                Map.of(
                        "mia:petrified_ship_body_small", 1,
                        "mia:petrified_ship_body_big", 1,
    ...
    public static void caveRaiderHutBottomIsOptionalAtEqualWeight(GameTestHelper helper) {
        assertTemplatePool(
                helper,
                "cave_raider_hut/start",
                "minecraft:empty",
                Map.of("mia:cave_raider_hut_top", 1));
        assertTemplatePool(
                helper,
                "cave_raider_hut/bottom",
                "minecraft:empty",
                Map.of(
    ...
    @PrefixGameTestTemplate(false)
    @GameTest(template = "gametest/amethyst_lamptube")
    public static void abyssSurfaceTemplatesArePackagedWithExpectedContracts(
            GameTestHelper helper) {
    ...
    public static void abyssSurfaceTemplatePoolsUseTheirStructureNames(GameTestHelper helper) {
        ...
        assertTemplatePool(
                helper, "fisherman_hut/starts", "minecraft:empty", Map.of("mia:fisherman_hut", 1));
        helper.succeed();
    }

    @PrefixGameTestTemplate(false)
    @GameTest(template = "gametest/amethyst_lamptube")
    public static void abyssStructureWorldgenKeepsApprovedBiomeAndPlacementContract(
            GameTestHelper helper) {
        ...
        Set<ResourceKey<Biome>> allAbyssBiomes =
                Set.of(
                        MiaBiomes.THE_ABYSS,
        ...
        Set<ResourceKey<Biome>> surfaceAbyssBiomes =
                Set.of(
                        MiaBiomes.SKYFOG_FOREST,
                        MiaBiomes.DENSE_SKYFOG_FOREST,
                        MiaBiomes.FOSSILIZED_FOREST,
        ...
        assertCaveFloorJigsawStructure(
                helper, "abyssal_ruins_01", "has_abyssal_ruins", "abyssal_ruins_01/starts", 9);
        assertCaveFloorJigsawStructure(
                helper, "abyssal_ruins_02", "has_abyssal_ruins", "abyssal_ruins_02/starts", 7);
        assertCaveFloorJigsawStructure(
                helper, "abyssal_ruins_03", "has_abyssal_ruins", "abyssal_ruins_03/starts", 9);
        assertCaveFloorJigsawStructure(
                helper, "abyssal_ruins_04", "has_abyssal_ruins", "abyssal_ruins_04/starts", 8);
        assertCaveFloorJigsawStructure(
                helper, "abyssal_ruins_05", "has_abyssal_ruins", "abyssal_ruins_05/starts", 9);
        assertCaveFloorJigsawStructure(
                helper, "abyssal_ruins_06", "has_abyssal_ruins", "abyssal_ruins_06/starts", 10);
        assertCaveFloorJigsawStructure(
                helper, "cave_raider_hut", "has_cave_raider_hut", "cave_raider_hut/start", 10);
        assertHeightCappedSurfaceJigsawStructure(
                helper,
                "ruined_cave_raider_hut",
                "has_ruined_cave_raider_hut",
                "ruined_cave_raider_hut/starts");
        assertHeightCappedSurfaceJigsawStructure(
                helper, "fisherman_hut", "has_fisherman_hut", "fisherman_hut/starts");

        assertRandomSpreadSet(
                helper,
                structureSetKey("abyssal_ruins"),
                16,
                4,
                70387321,
                Map.of(
        ...
        assertRandomSpreadSet(
                helper,
                structureSetKey("cave_raider_huts"),
                32,
                8,
                70387322,
                Map.of(
        ...
        assertRandomSpreadSet(
                helper,
                structureSetKey("fisherman_huts"),
                32,
                8,
                70387323,
    ...
    public static void miaJigsawHeightLimitRejectsBoundaryButAllowsBelow(GameTestHelper helper) {
        ...
        helper.assertTrue(
                atBoundary.findGenerationPoint(generationContext(helper)).isEmpty(),
        ...
        helper.assertTrue(
                belowBoundary.findGenerationPoint(generationContext(helper)).isPresent(),
    ...
    public static void petrifiedShipWorldgenUsesGreatFaultCaveContract(GameTestHelper helper) {
        TagKey<Biome> petrifiedShipBiomes =
                TagKey.create(Registries.BIOME, MiaUtil.miaId("has_petrified_ship"));
        ResourceKey<Structure> petrifiedShip =
                ResourceKey.create(Registries.STRUCTURE, MiaUtil.miaId("petrified_ship"));
        ResourceKey<StructureSet> petrifiedShips =
                ResourceKey.create(Registries.STRUCTURE_SET, MiaUtil.miaId("petrified_ships"));
        ...
        helper.assertValueEqual(
                structure.get("type").getAsString(), "mia:jigsaw", "petrified ship type");
        helper.assertValueEqual(
                structure.get("step").getAsString(),
                GenerationStep.Decoration.UNDERGROUND_STRUCTURES.getName(),
                "petrified ship generation step");
        helper.assertValueEqual(
                structure.get("terrain_adaptation").getAsString(),
                "beard_thin",
                "petrified ship terrain adaptation");
        helper.assertValueEqual(
                structure.get("start_pool").getAsString(),
                "mia:petrified_ship/start",
                "petrified ship start pool");
        ...
        helper.assertValueEqual(
                ...
                324,
                "petrified ship cave scan ceiling");
        helper.assertFalse(
                structure.has("project_start_to_heightmap"),
        ...
        helper.assertValueEqual(
                caveFloorSearch.getAsJsonObject("min_y").get("above_bottom").getAsInt(),
                16,
                "petrified ship cave scan lower bound");
        helper.assertValueEqual(
                caveFloorSearch.get("clearance").getAsInt(), 24, "petrified ship cave clearance");
        helper.assertValueEqual(
                caveFloorSearch.get("horizontal_samples").getAsInt(),
                4,
                "petrified ship cave column samples");
        helper.assertValueEqual(
                structure.get("max_distance_from_center").getAsInt(),
                128,
                "petrified ship maximum Jigsaw range");
        helper.assertFalse(
                structure.get("use_expansion_hack").getAsBoolean(),
                "petrified ship must not use the expansion hack");

        assertRandomSpreadSet(
                helper, petrifiedShips, 16, 4, 70387320, Map.of("mia:petrified_ship", 1));
        helper.succeed();
    }
    ...
    public static void miaJigsawCodecKeepsLegacyStructuresWithoutCaveSearch(GameTestHelper helper) {
        ...
        helper.assertValueEqual(
                stronghold.get("type").getAsString(),
                "mia:jigsaw",
                "abyss stronghold structure type");
        helper.assertFalse(
                stronghold.has("cave_floor_search"),
                "legacy Mia Jigsaw structures must not acquire cave-floor placement");
        helper.assertFalse(
                stronghold.has("project_start_to_heightmap"),
                "abyss stronghold must retain its original fixed-height placement");
        helper.assertValueEqual(
                stronghold.getAsJsonObject("start_height").get("absolute").getAsInt(),
                -50,
                "abyss stronghold fixed start Y");
        helper.assertValueEqual(
                stronghold.get("size").getAsInt(), 14, "abyss stronghold Jigsaw depth");
        helper.assertValueEqual(
                stronghold.get("max_distance_from_center").getAsInt(),
                230,
                "abyss stronghold maximum Jigsaw range");
        helper.succeed();
    }
    ...
    public static void caveFloorSearchFindsDrySturdyFloorWithClearance(GameTestHelper helper) {
        ...
        helper.assertValueEqual(
                ...
                List.of(11),
                "a sturdy floor below 24 dry air blocks must place the structure one block above"
                        + " the floor");
        helper.succeed();
    }
    ...
    private static List<Integer> findCaveFloorStarts(
            NoiseColumn column, int minY, int maxY, int clearance) {
    ...
    private static void assertJigsawTemplate(
        ...
        StructureTemplate template =
                helper.getLevel()
                        .getStructureManager()
                        .get(MiaUtil.miaId(path))
                        .orElseThrow(
                                () -> new AssertionError("missing structure template mia:" + path));
        helper.assertValueEqual(template.getSize(), expectedSize, path + " template size");
        helper.assertTrue(
                template.filterBlocks(
                                BlockPos.ZERO, new StructurePlaceSettings(), Blocks.STRUCTURE_BLOCK)
                        .isEmpty(),
                path + " must not contain authoring-only structure blocks");

        Set<JigsawContract> actualJigsaws =
                template
                                ...
                                info -> {
                                    if (info.nbt() == null) {
                                        throw new AssertionError(
                                                path
                                                        + " contains a Jigsaw without block-entity"
                                                        + " NBT at "
                                                        + info.pos());
                                    }
                                    return new JigsawContract(
    ...
    private static void assertTemplatePool(
        ...
        ResourceKey<StructureTemplatePool> key =
                ResourceKey.create(Registries.TEMPLATE_POOL, MiaUtil.miaId(path));
        ...
        JsonObject encoded =
                StructureTemplatePool.DIRECT_CODEC
                        ...
                        .getAsJsonObject();
        helper.assertValueEqual(
                encoded.get("fallback").getAsString(), expectedFallback, path + " fallback");

        Map<String, Integer> actualElements =
                encoded.getAsJsonArray("elements").asList().stream()
        ...
        StructureTemplate template =
                helper.getLevel()
                        ...
                        .orElseThrow(
                                () -> new AssertionError("missing structure template mia:" + path));
        int compassCount = 0;
        for (StructureTemplate.StructureBlockInfo pedestal :
                template.filterBlocks(
                        BlockPos.ZERO, new StructurePlaceSettings(), MiaBlocks.PEDESTAL.get())) {
    ...
    private static void assertOnlyItemFrames(
            GameTestHelper helper, String path, int expectedItemFrames) {
        ...
        try (InputStream stream = MiaGameTests.class.getResourceAsStream(resourcePath)) {
            ...
            helper.assertValueEqual(
                    entities.size(), expectedItemFrames, path + " intentional entity count");
            ...
            throw new AssertionError(
                    "failed to read structure template resource " + resourcePath, exception);
        }
    }
    ...
    private static Set<ResourceKey<Biome>> unionBiomeTags(
            net.minecraft.core.HolderGetter<Biome> biomes, TagKey<Biome>... tags) {
    ...
    private static void assertRandomSpreadSet(
        ...
        helper.assertValueEqual(
                placement.get("spacing").getAsInt(), spacing, key.location() + " spacing");
        helper.assertValueEqual(
                placement.get("separation").getAsInt(), separation, key.location() + " separation");
        helper.assertValueEqual(placement.get("salt").getAsInt(), salt, key.location() + " salt");

        Map<String, Integer> structures =
                structureSet.getAsJsonArray("structures").asList().stream()
                        .map(JsonElement::getAsJsonObject)
                        .collect(
                                Collectors.toMap(
                                        ...
                                        entry -> entry.get("weight").getAsInt()));
        helper.assertValueEqual(
                structures, expectedStructures, key.location() + " weighted structures");
    }
    ...
    private static void assertCaveFloorJigsawStructure(
        ...
        ResourceKey<Structure> key =
                ResourceKey.create(Registries.STRUCTURE, MiaUtil.miaId(structureName));
        JsonObject structure = encodeStructure(helper, key);
        helper.assertValueEqual(
                structure.get("type").getAsString(),
                "mia:jigsaw",
                structureName + " structure type");
        helper.assertValueEqual(
                structure.get("biomes").getAsString(),
                "#mia:" + biomeTag,
                structureName + " biome tag");
        helper.assertValueEqual(
                structure.get("step").getAsString(),
                GenerationStep.Decoration.UNDERGROUND_STRUCTURES.getName(),
                structureName + " generation step");
        helper.assertValueEqual(
                structure.get("terrain_adaptation").getAsString(),
                "beard_thin",
                structureName + " terrain adaptation");
        helper.assertValueEqual(
                structure.get("start_pool").getAsString(),
                "mia:" + startPool,
                structureName + " start pool");
        helper.assertValueEqual(
                structure.get("size").getAsInt(), 1, structureName + " Jigsaw depth");
        helper.assertValueEqual(
                structure.getAsJsonObject("start_height").get("absolute").getAsInt(),
                374,
                structureName + " maximum cave-floor origin");
        helper.assertFalse(
                structure.has("project_start_to_heightmap"),
        ...
        helper.assertValueEqual(
                caveFloorSearch.getAsJsonObject("min_y").get("above_bottom").getAsInt(),
                16,
                structureName + " cave-floor search lower bound");
        helper.assertValueEqual(
                caveFloorSearch.get("clearance").getAsInt(),
                clearance,
                structureName + " cave-floor clearance");
        helper.assertValueEqual(
                caveFloorSearch.get("horizontal_samples").getAsInt(),
                16,
                structureName + " sampled columns");
        helper.assertTrue(
                structure.get("use_expansion_hack").getAsBoolean(),
    ...
    private static void assertHeightCappedSurfaceJigsawStructure(
            GameTestHelper helper, String structureName, String biomeTag, String startPool) {
        ...
        helper.assertValueEqual(
                structure.get("step").getAsString(),
                GenerationStep.Decoration.SURFACE_STRUCTURES.getName(),
                structureName + " generation step");
        helper.assertValueEqual(
                structure.get("terrain_adaptation").getAsString(),
                "beard_thin",
        ...
        helper.assertValueEqual(
                structure.getAsJsonObject("start_height").get("absolute").getAsInt(),
                0,
                structureName + " surface projection offset");
        helper.assertValueEqual(
                structure.get("project_start_to_heightmap").getAsString(),
                "WORLD_SURFACE_WG",
                structureName + " surface heightmap");
        helper.assertValueEqual(
                structure.getAsJsonObject("max_start_y_exclusive").get("absolute").getAsInt(),
                375,
                structureName + " exclusive maximum start height");
        helper.assertTrue(
                structure.get("use_expansion_hack").getAsBoolean(),
                structureName + " expansion hack");
    }
    ...
    private static JsonObject encodeStructureSet(
            GameTestHelper helper, ResourceKey<StructureSet> key) {
        RegistryOps<JsonElement> ops =
                RegistryOps.create(JsonOps.INSTANCE, helper.getLevel().registryAccess());
        StructureSet structureSet =
                helper.getLevel()
                        ...
                        .getOrThrow(key)
                        .value();
        return StructureSet.DIRECT_CODEC
                .encodeStart(ops, structureSet)
                .getOrThrow()
                .getAsJsonObject();
    }
    ...
    private static MiaJigsawStructure decodeMiaJigsawStructure(
            GameTestHelper helper, JsonObject structure) {
        RegistryOps<JsonElement> ops =
                RegistryOps.create(JsonOps.INSTANCE, helper.getLevel().registryAccess());
        return (MiaJigsawStructure) Structure.DIRECT_CODEC.parse(ops, structure).getOrThrow();
    }

    private static void assertWindmillTemplate(
            GameTestHelper helper, String path, Vec3i expectedSize) {
        StructureTemplate template =
                helper.getLevel()
                        .getStructureManager()
