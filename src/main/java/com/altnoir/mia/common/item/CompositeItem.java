...
public class CompositeItem extends DiggerItem {
        ...
        if (player != null && hitresult.getType() == HitResult.Type.BLOCK) {
            ...
            if (hitresult instanceof BlockHitResult blockhitresult) {
                ...
                if (!player.level().isClientSide()
                        && player.level().getBlockEntity(blockpos)
                                instanceof MiaBrushableBlockEntity brushable) {
                    boolean success =
                            brushable.brush(player.level().getGameTime(), player, direction);
                    if (success) {
                        EquipmentSlot equipmentslot =
                                context.getItemInHand()
                                                .equals(player.getItemBySlot(EquipmentSlot.OFFHAND))
    ...
    private void spawnDustParticles(
        ...
        DustParticlesDelta brushitem$dustparticlesdelta =
                DustParticlesDelta.fromDirection(pos, direction);
        ...
        for (int k = 0; k < j; k++) {
            level.addParticle(
                    ...
                    brushitem$dustparticlesdelta.xd()
                            * (double) i
                            * 3.0
                            * level.getRandom().nextDouble(),
                    0.0,
                    brushitem$dustparticlesdelta.zd()
                            * (double) i
                            * 3.0
                            * level.getRandom().nextDouble());
        }
