...
public class LampTubeRecipeBuilder implements RecipeBuilder {
    ...
    public static LampTubeRecipeBuilder lampTube(
        ...
        return new LampTubeRecipeBuilder(
                SizedIngredient.of(ingredient, count), new ItemStack(result, resultCount));
    }

    public static LampTubeRecipeBuilder lampTube(
            TagKey<Item> tag, int count, ItemLike result, int resultCount) {
        return new LampTubeRecipeBuilder(
    ...
    public void save(@NotNull RecipeOutput recipeOutput, @NotNull String id) {
        ...
        ResourceLocation resourceLocation1 =
                ResourceLocation.fromNamespaceAndPath(MIA.MOD_ID, RECIPE_TYPE + "/" + id);
        if (resourceLocation1.equals(resourceLocation)) {
            throw new IllegalStateException(
                    "Recipe "
                            + id
                            + " should remove its 'save' argument as it is equal to default one");
        } else {
    ...
    public void save(RecipeOutput recipeOutput, @NotNull ResourceLocation id) {
        ...
        ResourceLocation advancementId =
                ResourceLocation.fromNamespaceAndPath(MIA.MOD_ID, id.getPath());
