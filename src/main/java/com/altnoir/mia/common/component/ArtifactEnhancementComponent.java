package com.altnoir.mia.common.component;
...
public class ArtifactEnhancementComponent {
    public static final ArtifactEnhancementComponent EMPTY =
            new ArtifactEnhancementComponent(0, List.of());
    ...
    public static final Codec<ArtifactEnhancementComponent> CODEC =
                    ...
                    inst ->
                            inst.group(
                                            Codec.INT
                                                    .fieldOf("level")
                                                    .forGetter(
                                                            ArtifactEnhancementComponent::getLevel),
                                            Codec.list(ArtifactStat.CODEC)
                                                    .fieldOf("modifiers")
                                                    .forGetter(
                                                            ArtifactEnhancementComponent
                                                                    ::getArtifactStats))
                                    .apply(inst, ArtifactEnhancementComponent::new));

    public static final StreamCodec<RegistryFriendlyByteBuf, ArtifactEnhancementComponent>
                ...
                newModifier =
                        new AttributeModifier(
                                ResourceLocation.fromNamespaceAndPath(
                                        curiosId.getNamespace(),
                                        curiosId.getPath() + "_" + newModifier.id().getPath()),
                                newModifier.amount(),
    ...
    public ArtifactEnhancementComponent addAttributeModifier(
        ...
        for (int i = 0; i < newList.size(); i++) {
            ...
            if (newList.get(i).attribute.equals(attribute)
                    && oldModifier.operation().equals(operation)) {
                AttributeModifier newModifier =
                        new AttributeModifier(
                                oldModifier.id(),
                                oldModifier.amount() + amount,
        ...
        if (newStat != null) {
            ...
            newList.add(
                    new ArtifactStat(
                            ...
                            new AttributeModifier(
                                    ResourceLocation.fromNamespaceAndPath(
                                            MIA.MOD_ID, getModifierKey(attribute, operation)),
                                    amount,
                                    operation)));
        }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof ArtifactEnhancementComponent that)) return false;
        return level == that.level && Objects.equals(modifiers, that.modifiers);
    }
    ...
    public record ArtifactStat(Holder<Attribute> attribute, AttributeModifier modifier) {
        public static final Codec<ArtifactStat> CODEC =
                RecordCodecBuilder.create(
                        inst ->
                                inst.group(
                                                Attribute.CODEC
                                                        .fieldOf("attribute")
                                                        .forGetter(ArtifactStat::attribute),
                                                AttributeModifier.CODEC
                                                        .fieldOf("modifier")
                                                        .forGetter(ArtifactStat::modifier))
